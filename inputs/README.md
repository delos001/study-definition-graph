...
That one rule, everything under `inputs/` is pinned, is what the Claude Code hook `.claude/hooks/deny_pinned_edits.py` and `src/sdgtools/find_unrecorded_files.py` both rely on. A new pinned source goes in a folder here and gets a manifest, and neither needs to be told.
