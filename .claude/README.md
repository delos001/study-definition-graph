...
| `hooks/` | The scripts the hooks run. Each follows `.claude/rules/writing_python_files.md` like every other Python file. `verify_headers`, run by the pre-commit hook `.githooks/pre-commit`, holds them to it. Their checks live under `validation/claude_hooks/`. |
