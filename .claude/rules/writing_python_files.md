...
A check is one pytest function that validates one thing, for one objective. The checks live under `validation/`, and they follow everything above plus these rules. The objectives and behavior cases they refer to are defined in `validation/README.md`.

- A check's objective is decided before the check is written, because the objective decides how the check is set up, what fixes a failure and who fixes it. Every check has exactly one objective. A check found to serve two is split when the two parts would have different fixes or different owners.
...
- Every check carries a `@code` marker holding its permanent id from `validation/validation_inventory.csv`, and an `@objective` marker naming its objective. A behavior check also carries `@positive`, for a working situation where the code is expected to succeed, or `@negative`, for a broken situation where the code is expected to refuse. A check of any other objective carries neither. `repo_tools/build_inventory.py` refuses a check whose markers break these rules.
...
- A behavior check touches nothing real and never downloads. Manifests and files are staged in a temporary folder through the fixtures in `validation/conftest.py`. Anything that downloads is replaced by a fake that serves bytes, or raises, per url. A workflow is called in-process through its `main()` with an argument list, never through a subprocess.
- A check of any other objective may read the real repo, because the real files are what it validates.
