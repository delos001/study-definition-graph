...
- A check finds one discrete issue, so that a failure names one thing to fix. It may look at several things to find that issue: a refusal and the message that explains it are one issue, and so are a report's verdict and the exit status it came from. Two situations staged in one check are two checks, for example a stale index and a missing one. A sentence that needs "and" to say what the check proves is a sign to look again, not a rule in itself. A situation several checks look at is staged once, in a fixture. The same check over several inputs uses `pytest.mark.parametrize`.
