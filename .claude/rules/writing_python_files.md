...
- Every check carries a `@code` marker holding its permanent id from `validation/validation_inventory.csv`, a `@category` marker and an `@objective` marker. There is no aspect marker: the aspect of quality is looked up from the objective, so it cannot disagree with it. A check that stages its own situation also carries `@positive`, for a working situation where the code is expected to succeed, or `@negative`, for a broken situation where the code is expected to refuse. A check that looks at something real carries neither. The case says how the check was set up, so any objective may carry one. `repo_tools/build_inventory.py` refuses a check whose markers break these rules.
