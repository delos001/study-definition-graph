...
## Whether the code does what it was built to do is its own objective, called functionality, decided 2026-09-22

The entry above, made earlier the same day, drew correctness and conformance on the data-quality split and closed with the sentence "Structure, format, layout, required fields and prescribed behaviour are therefore always conformance". Those last three words were the mistake. Once prescribed behaviour is conformance, every check that runs a piece of code and confirms it did what it was designed to do is conformance, and that is nearly every check in the suite. The count proved it: 442 of 456 checks carried the label, so the column separated almost nothing.

The split those words were derived from does not reach that far. DAMA's data-quality dimensions and ISO/IEC 25012 separate validity, whether a value complies with a rule, from accuracy, whether it reflects the thing it describes. Both are about data values, their format, their range and the set they are drawn from. Neither says anything about whether a function closes a connection when a query fails. Stretching validity across code behaviour is what swept the suite into one label.

The cost of leaving it was specific to this project rather than general. Conformance is the word CDISC uses, it is what the rules in `USDM_CORE_Rules.xlsx` decide, and Phase 3 of `PLAN.md` produces output that has to be held to it. Spending the same word on a check that a Python function pings a database before querying it blunts the term the project most needs to keep sharp.

A published standard covers the missing question, so the choice is **guided**. ISO/IEC 25010 names functional suitability as a quality characteristic in its own right, defined as whether a product provides functions that meet stated needs. The objective takes the shorter name `functionality`, and it sits under the `operation` aspect beside performance, reliability, security, compatibility, maintainability and portability, which are the remaining characteristics from that same standard.

One boundary separates the two, and the tools that enforce a rule are what make it worth stating. `repo_tools/verify_headers.py` enforces that every header holds eight fields in order. A check that stages a file with a field missing, runs the tool, and confirms it refuses with exit 17 is asking whether the tool works, so it is functionality. A check that runs the same tool over the real repository, and confirms every real header holds the eight fields, is asking whether those files have the shape the rule prescribes, so it is conformance. What decides is the thing being confirmed, not the rule in the background.
...
- The later stages run only when every check in the earlier stages passed. A check that failed, a check whose set-up or clean-up broke, and a check that was skipped each block them, and every check in the later stages is then skipped with a reason naming the stage and what blocked it.
...
- A skipped check blocks the later stages as a failure does, because the stages are ordered for confirmation. A later finding can be trusted only when the earlier stage confirmed the thing works, and a skipped check confirmed nothing. Treating a skip as a pass was considered and rejected for that reason. The cost is that one missing downloaded file skips every later check, including checks that never read it, and that is accepted because the skip reason names the cause and its fix.
...
## The validation machinery and the repo tools become installed packages under src/, decided 2026-09-23

The ordering between scripts decided today for issue #43 depends on grimp, an established library that maps which script imports which, reading every import in the project. It cannot read 12 of them. The repo tools are run as plain scripts, so they, and the checks that test them, import each other by bare file name, which works only because `pyproject.toml` adds `repo_tools/` to Python's search path. The validation machinery has the same problem. `validation/select_checks.py` is found only because the repo root is on the search path. The report writer is most of the 1,131 lines of `validation/conftest.py`. And both import their vocabulary from `repo_tools/build_inventory.py` by bare name. Two published guides cover the layout, so the choice is **guided**. The Python Packaging User Guide recommends that importable code live in packages under `src/`, installed and imported by name. pytest's own guidance keeps `conftest.py` for setups local to a folder of checks, treats code that changes how pytest runs as a plugin, and keeps tests in a folder outside the code they test.

- The validation machinery becomes `sdgval`, a package under `src/` beside `sdg`, loaded by pytest as a plugin. It holds the selection options, the labels a check carries, the blocking of checks whose downloaded file is missing or changed, the collection of outcomes, the report writer, the gate between aspects, the order between scripts, the import map and the refusal of loops.
- Each aspect's report has its own module inside `sdgval`, holding only what that aspect records, so one aspect's report can be designed or split further without touching the others. Three packages, one per aspect, were considered and rejected. Most of the machinery is shared by every aspect, the gate and the stage order exist only to relate one aspect to another, and a package is the unit Python installs and versions separately, while these parts are always installed together. The layout of the modules is settled when they are written.
- `repo_tools/build_inventory.py` moves into `sdgval`, because it holds the vocabulary the machinery selects and reports by. Its command keeps the name `build_inventory`.
- `validation/` keeps the checks, the shared setups they stage situations with, and the files that describe them. Reports are still written to `validation/reports/`, beside the inventory they join to and the dictionary that defines their columns.
- The repo tools become `sdgtools`, a package under `src/`, and each one is installed as a command, the way `sdg` provides `read_pdf`. They import each other by full name. Keeping them as scripts run with `python -m` was considered, and set aside once the one reason for it proved small: the pre-commit hook's first steps used only the standard library so that they ran without the `sdg` environment. That had already been relaxed for the step that runs ruff and mypy, which runs through `conda run` when the environment is not active, at about eight seconds against one. Every step of the hook now finds its command the same way.
- The two hooks stay where they are. `.githooks/pre-commit` and `.claude/hooks/` each live where the program that runs them looks. A hook is a guard that blocks one commit or one edit and keeps no record, which is a different job from validation, even where both enforce the same rule.
- Loops between scripts are ruled out across the repo, enforced before commit through grimp. pylint's `cyclic-import` check does the same job, but it misses loops when pylint runs in parallel, and a second tool would be a second opinion on what a script calls.

The names follow Python's naming guide, PEP 8, which asks for short, all-lowercase package names and discourages underscores. Both keep the `sdg` prefix, which marks them as the project's and keeps a generic name from clashing with another installed package. `sdgval` is short for validation, and because it is a short name it is written out as the validation package at its first use in each document. `sdgtools` is a whole word and fits every repo tool, including the two that confirm the setup rather than maintain anything. `sdg_validation` and `sdg_repo_tools` were considered and rejected, because each breaks PEP 8 twice, by length and by underscore. `validation` was ruled out because the checks folder is already imported under that name. `pytest_sdg` names the tool the package plugs into rather than what it does. `sdgcheck` was ruled out because a check is one validation check, and `sdgops` because it collides with the operation aspect.

Four established tools were weighed for the machinery and not taken, because their rule is not the project's. pytest-dependency and pytest-order work one named test at a time, and the gate and the order are stage-wide and script-wide. JUnit XML is built for build services, and the project has none. pytest-metadata reads the same values the report writer's existing calls read.

Left open: pytest's `importlib` import mode, which its guidance recommends for new projects and which bears on the remaining search-path additions; the pre-commit framework, an established way to manage git hooks; and whether `environment.yml` should fix library versions. grimp is added to `environment.yml` when the move is built.

## The operation report records the versions of what each check ran, decided 2026-09-23

An operation check confirms how a script runs, under any of the aspect's objectives: whether it does what it was built to do, how fast, how reliably, how securely, and the rest. Under every one of them the thing examined is the script, so the version that matters is the script's. The report recorded the fingerprint of the test file and the project's commit, and nothing about the script under test, so which version of it ran could be learned only from git. A check also runs more than its own script, because the script calls others, and the libraries it uses outside the project are fixed at no version in `environment.yml`. No standard covered the question, so the choice is **unguided**, although the format of its cells follows RFC 8259.

- The script under test is recorded three ways: the date it last changed, the id of that change, and its fingerprint. The date and the id are read from git when the report is written, so nothing is typed into a script's header and nothing can go stale. The date says roughly when, the id says exactly which version, and the fingerprint confirms without git that a copy in hand is the one that ran.
- The test file that holds the check is recorded the same three ways, because a check edited to test less can pass where the earlier one would have failed.
- A check that reads a fixture says so with a label, the way `@needs_pinned` does for a downloaded file, and only its row records that fixture, the same three ways. The name is written relative to `validation/fixtures/`. More than one fixture per check is expected once extraction and linking are staged, so the cell holds a list.
- Each row records the outside libraries its script uses, all the way down through the scripts it calls, with their installed versions, read through grimp and `importlib.metadata` in Python's standard library. Python's own libraries are left out, because `python_version` covers them.
- A check is judged on its own result. Its row also lists every script its script calls, at any depth, whose checks did not pass, in three columns: one for a check that failed, one for a check that broke and one for a check that was skipped. The lists are for the reviewer and change no outcome. A new outcome would need a new column, which forces the decision of whether it blocks and whether a reviewer should see it.
- A report run pulls in the checks of every script its selected scripts call, in the same aspect, so those columns always hold a real answer. Each check runs once per run, however many scripts call its script.
- Called scripts' checks run before their callers', at any depth. The order is a sort over grimp's map in the project's own code. pytest-order was weighed and not taken, because it orders one named test at a time and only warns when a label names a test that does not exist.
- Loops between scripts are ruled out, because no order exists for them. The refusal is built with the import map.
- Every cell holding several values is JSON. The CSV standard, RFC 4180, has no way to hold several values in one cell, and JSON is read back exactly by any language. A semicolon-separated form was rejected, partly because in Python's requirement notation a semicolon already introduces a condition.
- A report's file name starts with its aspect, as in `operation_2026-09-23_9524661.csv`, so `run_id` carries it and a copied file keeps it.

The columns `pinned_usdm_sha256`, `pinned_usdm_present` and `fixture_sha256s` are removed. The column names are settled when the layout is written into `validation/validation_report_dictionary.md`.

## A check's id is a plain unique key, and validation/ mirrors src/ one package per folder, decided 2026-09-23

A check's id was three letters naming the folder of the file it covered, then four digits. The move to three packages under `src/` broke that. The code of `repo_tools/` and much of `validation/` moves, so the prefixes `HRS` and `TST` stop naming where their checks' files live, and `src/sdgval/` would need a prefix of its own. The rule that an id never changes would have left moved checks carrying prefixes that name folders they are no longer in. The id is what data management calls a smart key, an identifier with meaning packed into it, and the established advice is that a permanent key should carry none, because meaning changes and a key must not. Where a check lives, what it covers and what it asks are already columns in `validation/validation_inventory.csv`, and those stay right because they are recomputed on every run. So the id needs only to be unique and permanent. The advice is general practice rather than a standard, so the choice is **unguided**.

- Every id is `SA` and five digits, as in `SA00001`. `SA` names suite A, and a separate suite of checks, if one is ever needed, starts at `SB00001`. A suite is created once and never changes, so its letter cannot go stale the way a folder can.
- The two letters are there so an id cannot be mistaken for anything else in the project. A single letter and five digits is the form of the NCI Thesaurus concept codes CDISC uses throughout its controlled terms, such as `C25457`. A hyphen after the letter would have kept the two apart, but it breaks an id in two when it is double-clicked to copy.
- Existing checks are renumbered once, before any report is filed, as the change from `SCR` to `HRS` was on 2026-09-11. After the first filed report an id never changes and is never reused.
- A test file at `validation/<package>/<path>` tests `src/<package>/<path>`, so the rule has one form for every installed package and a new package needs no change to it. The checks for `sdg` move down one level to match. Adding each new package as an exception to the old rule was considered and rejected, because the rule would grow with every package. Two exceptions stay: the Claude hook's checks in `validation/claude_hooks/`, because a folder whose name starts with a dot cannot be mirrored, and the checks for files that stay in `validation/` itself.

## The validation package splits by job, loads through pytest's entry point, and test files split by aspect, decided 2026-09-24

Moving the report writer out of `validation/conftest.py` required deciding how its machinery divides and how pytest finds it. The file held four kinds of code beside the setups the checks share: registering the labels a check carries, blocking a check whose downloaded file is missing or changed, collecting each check's outcome, and writing the report. The same review found that half the test files mix aspects of quality. Two choices are **guided** by pytest's documentation, which names the `pytest11` entry point as the way an installed package provides plugins and `test_*.py` as the files it collects by default. The rest are **unguided**.

- Each job has one file in `src/sdgval/`, named for what it does. `labels.py` declares the labels a check may carry and reads them off a check, which `select_checks.py` and the report both need. `select_checks.py` keeps only selection. `skip_rules.py` skips a check whose downloaded file is missing or changed, and receives the gate between aspects (#45) when it is built, because that also skips checks. `report.py` collects outcomes and writes the report, because outcomes exist only to feed it. The order between scripts (#43) gets a file of its own when it is built, because sorting is a different job from skipping.
- Grouping the labels and the skipping as configuration was considered and rejected, because one declares what a check may carry and the other decides at run time whether it runs, and a file named for both would tell a reader neither.
- `validation/conftest.py` keeps only the setups the checks share, which is the job pytest gives a conftest.
- pytest loads the plugins through the `pytest11` entry point in `pyproject.toml`, and `select_checks.py` moves to it from `addopts`. pytest then loads them wherever the project is installed, including the separate pytest processes the report's checks start.
...
## A report run writes the run's details in a file of their own, and each aspect's reports go in their own folder, decided 2026-09-25

A report repeated the run's details on every row: who ran it, the verdict, pytest's exit code and its cause, how many checks were collected and reported, the commit, and the tool versions. Every one of them is the same on every row, because a run writes a new file and never adds to an old one. Holding one run per row in a table of runs, joined to the table of check results on the run's id, is the standard way to store such data, but no standard names it for validation reports, so the choice is **unguided**.

- Each report run writes two files. The report holds one row per check. The run's own file, named as the report with `_run` added, holds one row with the run's details. The two join on `run_id`.
- The report keeps `run_id`, `run_started` and `selection` on every row. `run_started` stays because plotting results over time needs a date on each row, and `selection` stays as JSON so a reader has it at hand.
- In the run's own file, the selection is one column per way of narrowing a run, such as `selection_objective`. Those columns come last, so a new way adds a column at the right and every other column keeps its place. A column holds JSON when its way can hold several values.
- A single log appended on every run was considered and rejected. It would keep rows for development reports that are deleted rather than committed, it would be the one file every filing edits, and one bad write would damage the whole history. A combined view of runs is built from the run files instead, so it always matches the files that exist.
- Each report and its run file go in a folder named for the aspect inside `validation/reports/`, such as `validation/reports/technical/`. The writer knows the aspect, so the sorting costs nothing and cannot be skipped. Whether a report has been reviewed is left to the person, and a move to a reviewed folder is committed on its own, so git records who reviewed it and when.

## The technical report records each file's last change, and drops fingerprints and called scripts, decided 2026-09-25

The 2026-09-23 entry "The operation report records the versions of what each check ran" set out what the technical report records. Reviewing it before building the rest changed four parts. No standard covered the question, so the choice is **unguided**.

- A file's version is its last change in git, recorded as a date and a change id, for the script under test, the test file and each fixture a check names. The fingerprint was dropped for all three, and `check_file_sha256` with them. A report run starts only on a folder that matches its commit, so the change id and the commit already say exactly which version ran, and a fingerprint helps only a reader holding a loose copy of a file with no git.
