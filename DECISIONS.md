...
The names were chosen against Python's naming guide, PEP 8, which discourages but allows underscores. `validation` was ruled out because the checks folder is already imported under that name. `pytest_sdg` names the tool the package plugs into rather than what it does. `sdgval` is an abbreviation a reader would have to look up. `sdg_repo_tools` keeps the name readers already know, with the project prefix, because a bare `repo_tools` is generic enough that another installed package could share it.
