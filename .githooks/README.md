...
| `python scripts/verify_headers.py --quiet` | Refuses the commit if any Python file under `src/sdg/` or `scripts/` lacks the full header block, has its fields out of order, or has a Date that is not a plain calendar date. |
