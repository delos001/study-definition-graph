...
The header and index checks use only the standard library, so they run from any terminal. The ruff and mypy check needs the `sdg` environment; its script finds the tools on the path when that environment is active and runs them through `conda run` when it is not, which is slower but needs no set-up.
