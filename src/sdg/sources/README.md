...
Get and keep the pipeline's inputs. Three orchestrators, each a sequence of the pieces below; the pieces do one thing each and are shared.
...
| `verify_pinned.py` | Hands a pipeline stage one pinned file after proving it is the recorded one. Header done; code to be rewritten on the pieces. |
...
| `write_manifests.py` | Writes or updates one entry. Not written yet. |
