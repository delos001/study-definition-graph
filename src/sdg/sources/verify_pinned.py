...
Description: Hands a pipeline stage the path and identity of a pinned file after
             proving it is the file recorded in the manifest.
...
Usage:       Not run directly; imported.
             from sdg.sources.verify_pinned import verify_pinned
...
def check_entry(entry: dict) -> tuple[str, str]:
    ...
    if not local or not recorded_hash:
        return (
            "malformed",
            f"{entry.get('name', '?')}: entry has no local path or no sha256",
        )
    ...
    if recorded_size is not None and actual_size != recorded_size:
        return (
            "mismatch",
            f"{local}: size {actual_size} bytes, manifest says {recorded_size}",
        )
    ...
    if actual_hash != recorded_hash:
        return (
            "mismatch",
            f"{local}: sha256 {actual_hash[:16]}..., manifest says {recorded_hash[:16]}...",
        )
