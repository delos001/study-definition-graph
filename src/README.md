...
Code here is used in two ways. Other code imports it, for example `from sdg.sources import manifests`. Workflows are also run as programs, for example `python -m sdg.sources.acquire_sources`. The tools a person runs beside the pipeline live in `scripts/`, and the checks that prove this code works live in `tests/`.
