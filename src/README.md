...
Code here is used in two ways. Other code imports it, for example `from sdg.sources import manifests`. Workflows are also run as programs, for example `python -m sdg.sources.acquire_sources`. The tools a person runs beside the pipeline live in `scripts/`, and the checks that prove this code works live in `validation/`.
...
`docs/sdg_file_inventory.md` lists every file in the package, folder by folder, as a workflow or a step, with what each uses.
