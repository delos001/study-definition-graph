...
Description: Generates repo_tools/README.md from the header block of every script
             in repo_tools/, so the index of what each one does is derived rather
             than maintained.
...
Inputs:      repo_tools/*.py   (read-only, parsed rather than imported)

Outputs:     repo_tools/README.md, overwritten in full on every run.
             Writes nothing under --check.

Usage:       python repo_tools/build_index.py
...
Exit codes:  0   success (the index was written, or --check found it current)
             ...
             15  repo_tools/README.md is stale or missing (--check only)
...
# on the next run.
GENERATED_NOTICE = (
    "Generated by `python repo_tools/build_index.py`. Do not edit by hand: "
...
def render(entries: list[tuple[str, dict[str, list[str]]]]) -> str:
    """Build the whole of repo_tools/README.md from the parsed headers.
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Generate repo_tools/README.md from each script's header block."
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    if args.check:
        ...
        if current == generated:
            say(f"repo_tools/README.md is current, {len(entries)} script(s)")
            return 0
