...
The sharpest instance is the Schedule of Activities: a grid where rows are activities, columns are visits. The grid is a lossy rendering whereas the real structure is a directed graph of scheduled activities and decision points connected by timing rules.  Rebuilding it is graph reconstruction from a flattened two-dimensional projection, not table parsing. Much of what the printed schedule expresses has no cell in the table: a footnote, an asterisk, or a line of prose such as "only if ALT above twice the upper limit of normal", "repeat every 21 days until progression", or "may be performed within 7 days prior". Each of those has to be tied to the right element of the table, and the document does not say how.
...
- One tool, ProtocolMiner, produced accurate, detailed timelines for **22 of 29** legacy schedules, about 76%, with minor errors in the rest. It mapped them into USDM structures and into the Fast Healthcare Interoperability Resources standard (FHIR). This is a single study of a single tool, so the confidence is low to moderate, but it shows that prior art exists: this is not unexplored ground. [6]
