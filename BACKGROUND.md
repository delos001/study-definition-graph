...
A clinical trial protocol contains a Schedule of Activities: a grid where rows are activities, columns are visits, and an X means the activity happens at that visit.

The grid is a lossy rendering. The real structure is a directed graph of scheduled activities and decision points connected by timing rules. Reconstructing it is graph reconstruction from a flattened two-dimensional projection, not table parsing.

The consequence that defines the project: everything the printed schedule expresses as a footnote, an asterisk, or prose ("only if ALT above twice the upper limit of normal", "repeat every 21 days until progression", "may be performed within 7 days prior") has **no cell in the table**.
...
1. **Footnotes containing conditionals.** A footnote marker on a cell that changes whether or when the activity happens.
